...
return {
  ...
  lazy = false,
  -- require("gruvbox") below fails until gruvbox.nvim is on runtimepath and its config has run
  -- (verified empirically -- require("gruvbox") errors "module not found" without it, even
  -- with the theme below never reading vim.g.colors_name the way the "auto" theme would have).
  -- mini.icons supplies the fake nvim-web-devicons module lualine's filetype component looks
  -- for (lua/plugins/miniicons.lua).
  dependencies = { "gruvbox.nvim", "mini.icons" },
  config = function()
    ...
    -- replacing only each mode's `a` section background: mini.tabline's current-tab highlight
    -- is already a solid bright_green fill (lua/plugins/gruvbox.lua's MiniTablineCurrent), so a
    -- green mode block would read as the same colored chunk repeated one line below it.
    local theme = {
      normal = {
