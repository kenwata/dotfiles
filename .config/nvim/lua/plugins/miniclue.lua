...
-- configuration data), so plan.md's startup-load exception does not apply.
--
-- Unlike claudecode.nvim and mini.pick, the entry points here are not real commands: the
-- 19 stubs below exist only to load mini.clue and then replay the key that triggered them,
...
local function on_trigger(trigger)
  return function()
    delete_stubs()
    lazy.require("mini.clue", "mini.clue", setup)
