...
return {
  ...
  lazy = false,
  config = function()
    -- mini.icons itself draws nothing; it only answers "what glyph and highlight group for
    -- this file/extension/filetype/directory/lsp-kind/os". mini.pick and mini.files already
    -- look for _G.MiniIcons, so setup() alone makes their generic single-character icons
    -- become per-type glyphs, with no change needed in either plugin's own config.
    --
    -- style is the only option written out. mini.icons accepts five more (default, filetype,
    -- extension, file, directory, lsp, os -- per-category overrides of the built-in icon
    -- table); none are set here, so every category keeps mini.icons' own defaults.
