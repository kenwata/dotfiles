...
**Ensure the inbox watcher is running first.** Before processing any subcommand below, if the project's delivery mode is `monitor` or `both` (check via `~/.agents/skills/agmsg/scripts/delivery.sh status claude-code "$(pwd)"`), make sure this session has a live watcher. Two styles exist, depending on whether the Monitor tool is available in this environment (it is NOT offered on e.g. Claude Code with Bedrock — the SessionStart directive you received tells you which style applies):

- **Monitor style (default):** check whether this session already has an `agmsg inbox stream` Monitor task in its TaskList. If not, invoke the Monitor tool now:
  - command: `~/.agents/skills/agmsg/scripts/watch.sh $CLAUDE_CODE_SESSION_ID "$(pwd)" claude-code`
  - description: `agmsg inbox stream`
  - persistent: true
- **Bash fallback (no Monitor tool):** check whether a `watch.sh --once` background shell is already running in this session. If not, run with the Bash tool, run_in_background: true:
  - `~/.agents/skills/agmsg/scripts/watch.sh --once $CLAUDE_CODE_SESSION_ID "$(pwd)" claude-code`
  - Each time that task exits, its output lines are incoming messages: react to them, then IMMEDIATELY re-arm the same command (Bash, run_in_background: true) — every time, even mid-task. A persisted cursor guarantees nothing is lost between re-arms.
...
5. **Switch receive too — exclusive role mode.**
   ...
   d. Invoke a fresh Monitor regardless of whether step b or c applied:
      ...
      - persistent: true
   e. **Bash fallback environments** (no Monitor tool): skip steps a–d. Instead run with the Bash tool, run_in_background: true:
      - `~/.agents/skills/agmsg/scripts/watch.sh --once $CLAUDE_CODE_SESSION_ID "$(pwd)" claude-code <name>`
      The new watcher automatically replaces any previous broad watcher for this session (same pidfile) — the old background task will exit with no output; do NOT re-arm the old unfiltered command when that happens. From now on, re-arm with this `<name>`-filtered command every time it exits.
   The trailing `<name>` argument to `watch.sh` restricts the subscription to messages addressed to `<name>` only — other roles' inbound messages stop reaching this session until another `actas` or session end.
...
3. If the session's active FROM was `<name>`, clear that state. Then:
   ...
   e. **Bash fallback environments** (no Monitor tool): skip steps a–d. Instead run with the Bash tool, run_in_background: true:
      - `~/.agents/skills/agmsg/scripts/watch.sh --once $CLAUDE_CODE_SESSION_ID "$(pwd)" claude-code`
      The new watcher replaces the previous filtered one automatically; re-arm this unfiltered command from now on.
