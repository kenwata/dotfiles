...
- The context-budget hook (`hooks/context-budget.sh`, shared logic `~/.claude/hooks/context-budget.mjs`) reads Codex usage from the tail of the rollout at `transcript_path` (the last `token_count` event), because Codex has no command status line to report it. The Codex thresholds default to 60% / 70% of the model context window (Claude Code: 70% / 80%); both live in `~/.config/claude-task-loop/config.json`. Codex compaction is recorded from `SessionStart` with `source: "compact"`, whereas Claude Code uses `PostCompact`.
...
- A new file under `hooks/` is live in `user-hooks.json` (linked as `~/.codex/hooks.json`) as soon as it is registered, but its script reaches `~/.codex/hooks/` only when `install.sh` links it. Re-run `install.sh` in the same change, or every matching Codex event fails to start the missing script.
