...
This directory is the version-controlled source for personal Codex configuration. `~/.codex` remains a real runtime directory; `install.sh` links declarative files and merges `user-config.toml` into the writable runtime `config.toml`, so authentication, sessions, history, caches, system skills, plugins, hook trust, and local state survive dotfiles installation.
...
| `agents/*.md` | `agents/*.toml` | Four roles are ported; `advisor` is added as a fresh-context replacement for Claude Advisor |
