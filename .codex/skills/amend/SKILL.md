...
- Treat the task ID supplied with `$amend` as the command's `$ARGUMENTS`. With a task ID, the hole record under "仕掛かり中" in `HANDOFF.md` is the primary source; do not reconstruct the situation from the whole design document or from chat history. Without a task ID, the user's instruction in this conversation is the primary source.
