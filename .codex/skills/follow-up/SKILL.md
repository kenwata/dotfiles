...
Read `~/.claude/commands/follow-up.md`, `~/.claude/templates/model-routing.md`, and the sections of `~/.claude/templates/BLUEPRINT.md` the command references. Preserve its three operating modes, its completion-condition change inspection (give the `diff_reviewer` the before and after text of every completion-condition block rewritten in the range), its goal reconciliation against the `plan.md` phase each design points at (collected every run, whether or not `plan.md` changed; give the `diff_reviewer` that excerpt verbatim and route its findings to the user instead of correcting them), its handling of abolished (`[-]`) tasks, its rule that a plan row stays `[ ]` while the design still lists a `未分解` stage, first-parent commit-trailer checkpoint, bootstrap behavior, cross-task standard discovery, mechanical checks, state-document updates, and distinction between automatic correction and new user decisions.
...
- Run the canonical command's collection of pending confirmation items (`要確認`): ask every item that existed when the run started through `request_user_input`, write answers in steps 5–6 only, record each decision as one line in `docs/decisions.md`, and route any resulting work to the next action instead of doing it. Withdraw a question only when the item is measurably already settled; never settle it yourself. If no answer can be obtained (for example a non-interactive run), still create the checkpoint, carry the items over verbatim, and make settling them the next action.
- Do not rewrite completion conditions or mark tasks `[-]` during follow-up; route such changes to the `amend` skill as a user decision.
