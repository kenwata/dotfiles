...
4. **検証**: BLUEPRINT §11 の生成後検証(ポインタ実在確認・rules の `paths:` frontmatter 一致確認・循環参照の不在確認・git repo であることの確認)を実施する。Glob/Grep/Read で確認し、不一致があれば報告前に修正する。
5. **初期コミット**(BLUEPRINT §6 の書式・§9 に従う): 新規に `git init` した場合は、生成ファイルのみを個別パス指定で `git add` し自動コミット。既存 repo の場合は生成ファイルをコミットしてよいか確認し、拒否されたらスキップとして報告。メッセージ例:
   ...
   対応: rules / CLAUDE.md / HANDOFF.md / docs/decisions.md を BLUEPRINT に従い生成
   ```
