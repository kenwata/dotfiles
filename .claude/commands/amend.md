...
4. **反証**: `proposal-reviewer` エージェントを **1 回だけ** 起動し、改訂案・穴の記録・該当節の原文・影響タスクの原文・`plan.md` の抜粋の原文(手順 1 で読んだもの。無ければ設計書の「目的(アウトカム)」、それも無ければ照合先なしと明記する)を自己完結のプロンプトで渡す。特に (a)完了条件の変更が、実装に合わせた緩和になっていないか (b)追加タスクが本当に必要か、既存タスクの書き換えで表せないか (c)段の判定が正しいか(目的・スコープ・`plan.md` のフェーズ構成に触れていないか) (d)改訂後のタスク群でも `plan.md` の該当フェーズの目標に届くか(部分改訂の積み重ねで目標から離れていないか) (e)改訂後の各タスクが、会話履歴なしの新セッションがそれだけ読んで着手できる自己完結記述のままか、を反証させる。返ってきた指摘は実測で精査してから改訂案へ反映し、再レビューはしない。
