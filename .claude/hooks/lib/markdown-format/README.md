...
## 2026-09-16 の実害バグ修正(大きなファイルへの Edit が全体モードに落ちる)

上記の事故では、編集したのはファイル冒頭の 3 行だけなのに、離れた行まで整形されていた。
`scope.mjs` の「編集行のみ」の限定が効いていなかった。

原因: `cli.mjs` の `readStdinJson` が、読み取りの前に `process.stdin.isTTY` を参照していた。
Node は `process.stdin` に触れた時点で fd 0 のパイプをノンブロッキングにするため、パイプの
バッファ(約 64KB)を超える入力では `readFileSync(0)` が `EAGAIN` を投げることがある
(実測で 3 回中 2 回)。この例外を「stdin 無し」と同じ扱いにしていたため、全体モードに
落ちた。PostToolUse の入力は編集前のファイル全文(`tool_response.originalFile`)を含むので、
大きなファイルへの Edit ほど起きやすい。

修正: TTY 判定を `node:tty` の `isatty(0)` に替えて `process.stdin` に触れない。読み取り自体が
失敗した場合は「stdin 無し」と区別し、範囲を決められないため何もせず終了する
(`new_string` が見つからない場合と同じフェイルオープン)。`test/cli.test.mjs` に、600KB の
`originalFile` を含む入力を 5 回渡して未編集行が変わらないことを確かめるテストを追加した。
