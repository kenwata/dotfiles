...
2. 選択を聞いていないか → 整合性と正しさを取る「べき論」(既存の設計原則・先例との整合、事実との一致)で結論を 1 つ出し、可逆で権限内なら実行して作業を続ける。選択肢を並べて選ばせるだけで終わらない。判断軸は仕組みとしての完成度であり、手直しの少なさではない(~/.claude/CLAUDE.md の Decision)。ただし、最終的にすべてやる前提で作業の順序を入れ替え、アウトカムに近い作業を先に置くことは手抜きではない。最終状態の品質が同じなら、アウトカムに早く届く順序を選ぶ(後回しにした作業は TODO 等の追跡される場所に記録する)。
