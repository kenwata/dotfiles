...
- **目標への照合**: `/breakdown`・`/amend`・`/follow-up` は、直前の中間文書(設計書・完了条件)だけで
  なく、`plan.md` の該当フェーズ(設計書の「全体構想」行が指す節の中の、そのフェーズ見出しの本文)と
  `plan.md` 冒頭の目標に照らして反証する。`plan.md` 全体は読まない。中間文書は目標へ至る手段であり
  正ではない — 各段が直前の文書だけを見ると、段ごとに筋が通っていても目標からのずれが伝言のように
  たまり、どの検査にも掛からない。ずれが見つかったら中間文書を黙って正にせず、実装・中間文書を戻す
  (部分改訂なら `/amend`、目的・スコープなら `/elaborate`)か、`plan.md` を改める(上の「plan.md の
  更新契機」の規則で `docs/decisions.md` に 1 行)かを利用者判断へ回す。`plan.md` が無い(「全体構想:
  なし」)場合は設計書の「目的(アウトカム)」が照合先、それも無ければ照合先なしと報告に明記する
...
- **1 タスク = 1 セッションは機構で回す**: 複数の T を続ける時は、利用者が手で打っていた `/clear` → `/execute-task` を
  `hooks/lib/task-loop/` のループが代行する(1 つのセッションに T を積まない)。次に回す T は `HANDOFF.md` の次の一手で決め
  (`TODO.md` の並びは実行順ではない)、1 回の起動は `/follow-up` の 1 区間で終える。次の T へ進む判定は成果物だけ
  ...
  利用者の答えを待つ(上限は既定 24 時間。待った時間はタスクの制限時間に数えない)。計画工程のコマンドは、着地した時と `/elaborate` へ回す時の結論を `HANDOFF.md` の次の一手と
...
4. `node ~/.claude/hooks/lib/markdown-header-rule-check/cli.mjs <プロジェクトルート>`(冒頭に規約コメントを持つ文書が規約どおりかの検査。`/follow-up` 機械チェックと同じ)を
...
以下6点を機械的に確認してから §2 の報告(作成/スキップ一覧)を行う。手作業の目視確認
...
6. **ルールが要求するツール設定の実在確認**: コピーした各言語ルールの Code Style 節が要求する
   formatter・linter について、設定ファイルと依存(`package.json` の devDependencies、
   `pyproject.toml` の `[tool.ruff]`、`.scalafmt.conf` 等)が実在し、行幅が 100 に設定されて
   いるかを確認する。欠けていれば「未整備のツール設定」として報告に列挙し、導入はしない
   (既存コードの全体再整形という大きな差分を生むため、利用者の判断に回す)
