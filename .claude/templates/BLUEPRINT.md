...
**将来の拡張継ぎ手**: TODO.md の表(計画 / #<n>-<m> / T<n> / タスク / 難易度 / 状態)の
列構成はそのまま関係スキーマになる。クエリ性・不変条件の機械保証が必要になった時点で
`.claude/tasks.yaml` を正とし、`TODO.md` を `yq` 生成のビューへ移す。SQLite は
`git diff` でタスク定義の変更履歴が追えなくなるため、複数プロジェクト横断のダッシュボードが
必要になるまで見送る。
