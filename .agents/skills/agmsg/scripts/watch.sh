...
#
# Usage: watch.sh [--once] <session_id> <project_path> <agent_type> [active_name]
#
# Behavior:
...
#   - With --once, exits 0 right after emitting the first batch of new
#     messages. Intended for runtimes without a Monitor tool (e.g. Claude
#     Code on Bedrock): the host agent runs it as a background Bash task,
#     gets woken by its exit, processes the output, and re-arms it.
...
#     historical messages. A cursor file (run/cursor.<session_id>) persists
#     the last delivered id across invocations, so messages arriving while
#     no watcher is running (the --once re-arm gap) are never lost.
...
# --once: exit after the first delivered batch (re-arm style, see header).
ONCE=0
if [ "${1:-}" = "--once" ]; then
  ONCE=1
  shift
...
SESSION_ID="${1:?Usage: watch.sh [--once] <session_id> <project_path> <agent_type> [active_name]}"
...
# Last delivered message id for this session. Survives watcher restarts so
# --once re-arms (and monitor relaunches) resume where the previous run
# stopped instead of skipping anything sent in between. Cleaned up by
# session-end.sh together with the pidfile.
CURSORFILE="$RUN_DIR/cursor.$SESSION_ID"
...
# Non-zero exits on signals so the --once re-arm loop can distinguish
# "killed" (non-zero → do NOT re-arm) from "delivered a batch" (0 → re-arm).
# SIGINT=130, SIGTERM/HUP=143 follow the 128+signum convention.
trap 'exit 130' INT
...
if [ -z "$PAIRS" ]; then
  ...
  # Exit non-zero so the --once re-arm loop knows NOT to re-arm.
  # 75 = EX_TEMPFAIL — transient condition, not a delivery.
  exit 75
fi
...
# Get the starting watermark. A persisted cursor from a previous run of this
# session wins (resume without loss); otherwise baseline at MAX(id) so no
# historical messages are replayed. Missing DB is OK — we'll just block until
# it appears.
LAST=""
if [ -f "$CURSORFILE" ]; then
  LAST="$(cat "$CURSORFILE" 2>/dev/null || true)"
...
while true; do
  if [ -f "$DB" ]; then
    ...
    if [ -n "$ROWS" ]; then
      ...
      # Persist the cursor after every delivered batch so a successor run
      # (or a --once re-arm) resumes exactly past what was already shown.
      printf '%s\n' "$LAST" > "$CURSORFILE" 2>/dev/null || true
      if [ "$ONCE" = "1" ]; then
        exit 0
      fi
    fi
